	...
	export default {
		...
		methods: {
			...
			requirePhoto(idPhoto) {
				return `../src/assets/images/${this.user.photos[this.user.photosId.indexOf(idPhoto)]
					}`;
			},
	...
	<div class="background">
		...
		<div class="col-md-9 col-lg-10">
			<div class="d-flex justify-content-center align-items-center h-70">
				<div class="card2 bg-white text-dark rounded-5 position-relative">
					<div class="card-body p-5 text-center">
						...
						<div class="info">
							<div>
								<a href="javascript:" class="text-muted mb-1 larger-text" style="text-decoration: none"
									@click="
										followersView(
											this.userId,
											this.user?.username
						...
						<div v-if="
							...
							">
							<div class="gap-3">
								<button class="btn btn-primary btn-block rounded-pill larger-text" type="submit"
									@click="banUser(this.userId, this.user?.id)" style="background-color: #d10606f5">
									{{ this.banTag ? "Unban" : "Ban" }}
								</button>
								<button class="btn btn-primary btn-block rounded-pill larger-text" type="submit" @click="
			...
			<div v-if="parseInt(this.user?.id) === parseInt(this.userId)"
				...
				<router-link :to="{
						name: 'UploadPhoto',
					}" class="btn btn-primary rounded-pill larger-text" style="background-color: #2e4a78">Upload Photo</router-link>
				<router-link :to="{
						name: 'SetMyUsername',
					}" class="btn btn-primary rounded-pill larger-text" style="background-color: #2e4a78">Update my Username
				</router-link>
			</div>

			<div class="photo-grid d-flex flex-wrap justify-content-center align-items-center h-80">
				<div class="col-9 col-sm-6 col-md-4 col-lg-3 mb-5" v-for="photo in this.user?.photosId" :key="photo.id">
					<div class="photo-card bg-white text-dark rounded-5 position-relative">
						<div v-if="
							...
							<div class="dropdown position-absolute top-0 end-0 p-2">
								...
								<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
									<li>
										<button class="dropdown-item" @click="deletePhoto(photo)">
											Delete Photo
										</button>
									</li>
								</ul>
						...
						<div class="card-body">
							<div class="d-flex justify-content-between align-items-center mb-2">
								...
								<div class="comments" v-if="comments[photo]">
									<svg class="feather">
										<use href="/feather-sprite-v4.29.0.svg#comment" />
									</svg>
							...
							<div class="comment-section">
								<div class="comment" v-for="comment in comments[photo]" :key="comment.id">
									...
									<button v-if="
											...
											parseInt(this.userId)
										" class="btn btn-link text-danger m1-auto" @click="
								...
								<div class="mt-3">
									<input type="text" v-model="newComment[photo]" class="form-control"
										placeholder="Add a comment..." @keyup.enter="commentPhoto(photo)" />
								</div>
							</div>
						</div>
					</div>
